            ...
            ConfigOptions.key("scan.startup.mode")
                    ...
                    .withDescription(
                            "Optional startup mode for SQL Server CDC consumer, valid enumerations are "
                                    + "\"initial\", \"latest-offset\", \"snapshot\" or \"timestamp\"");
            ...
            ConfigOptions.key("chunk-key.even-distribution.factor.upper-bound")
                    ...
                    .withDescription(
                            "The upper bound of chunk key distribution factor. The distribution factor is used to determine whether the"
                                    ...
                                    + " and the query SQL Server for splitting would happen when it is uneven."
            ...
            ConfigOptions.key("metadata.list")
                    ...
                    .withDescription(
                            "List of readable metadata from SourceRecord to be passed to downstream, split by `,`. "
                                    + "Available readable metadata are: database_name, schema_name, table_name, op_ts.");
            ...
            ConfigOptions.key("scan.newly-added-table.enabled")
                    ...
                    .withDescription(
                            "Whether to scan the newly added tables or not, by default is false. "
                                    + "This option is only useful when we start the job from a savepoint/checkpoint.");
}
