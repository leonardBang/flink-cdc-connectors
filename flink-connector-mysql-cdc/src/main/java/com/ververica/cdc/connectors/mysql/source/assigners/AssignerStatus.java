...
public enum AssignerStatus {
    ...
    public AssignerStatus onFinish() {
        throw new IllegalStateException(
                format(
                        "Invalid call, assigner under {} state can not call onFinish()",
    ...
    public AssignerStatus suspend() {
        throw new IllegalStateException(
                format(
                        "Invalid call, assigner under {} state can not call suspend()",
                        fromStatusCode(this.getStatusCode())));
    }
    ...
    public AssignerStatus wakeup() {
        throw new IllegalStateException(
                format(
                        "Invalid call, assigner under {} state can not call wakeup()",
                        fromStatusCode(this.getStatusCode())));
    }
    ...
    public static AssignerStatus fromStatusCode(int statusCode) {
        switch (statusCode) {
            ...
            default:
                throw new IllegalStateException(
                        format(
                                "Invalid status code {},the valid code range is [0, 4]",
                                statusCode));
        }
    }
